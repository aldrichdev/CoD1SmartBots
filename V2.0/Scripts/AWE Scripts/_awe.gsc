...
monitorsprinting()
{
	...
	while (isAlive(self) && self.sessionstate == "playing")
	{
		...
		// ADDED BY KRAKEN - 08/20/2016
		// Check for bots (need to disable sprint for them)
		isBot = maps\mp\gametypes\_awe_bots::isBot(self);

		// No sprinting if parchuting or under spawnprotection (with disabled weapon)
		if(isBot || (isdefined(self.awe_invulnerable) && level.awe_spawnprotectiondisableweapon) || isdefined(self.awe_isparachuting))
			continue;
